class SQLivro:
    ...
    _CAMPOS_UPDATE =[_COL_TITULO, _COL_AUTOR,_COL_GENERO, _COL_QUANTIDADE_ESTOQUE, _COL_PRECO, _COL_DATA_PUBLICACAO]
    _CAMPOS_VERIFICAR_PUT =[_COL_QUANTIDADE_ESTOQUE, _COL_PRECO]
    ...
    _SELECT_BY_TITULO_GENERO_AUTOR_DATA = f"SELECT * FROM {_TABLE_NAME} WHERE LOWER({_COL_TITULO}) ILIKE LOWER(%s) AND  LOWER({_COL_GENERO}) ILIKE LOWER(%s) AND LOWER({_COL_AUTOR}) ILIKE LOWER(%s) AND {_COL_DATA_PUBLICACAO} = %s"
    ...
    _UPDATE_PRECO_BY_ID = f"UPDATE {_TABLE_NAME} SET {_COL_PRECO} = %s WHERE {_COL_ID} = %s;"
    _ADICONAR_ESTOQUE = f"UPDATE {_TABLE_NAME} SET {_COL_QUANTIDADE_ESTOQUE} = {_COL_QUANTIDADE_ESTOQUE} + %s WHERE {_COL_ID} = %s;"
    _REMOVER_ESTOQUE = f"UPDATE {_TABLE_NAME} SET {_COL_QUANTIDADE_ESTOQUE} = {_COL_QUANTIDADE_ESTOQUE} - %s WHERE {_COL_ID} = %s;"
